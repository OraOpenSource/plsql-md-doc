...
pmd.saveToFile = function(objs){
  ...
  objs.forEach(function(obj){
    ...
    fs.writeFileSync(path.resolve(obj.folder.outputPath,obj.fileData.name + '.md'), markdown);
  });
}//saveToFile
...
pmd.generateToc = function(config, objs){
  ...
  if (config.toc.template){
    ...
    templateContent = fs.readFileSync(config.toc.template,'utf8'),
    template = Handlebars.compile(templateContent);
    markdown = template(indexData);

    fs.writeFileSync(path.resolve(config.folders[0].outputPath, config.toc.fileName), markdown);
