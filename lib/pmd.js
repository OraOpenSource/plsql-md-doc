...
pmd.saveToFile = function(config, objs){
  ...
  objs.forEach(function(obj){
    ...
    var extname = path.extname(config.folders[0].template);

    if (debug.debug){
      debug.logFile(obj.fileData.name + extname, markdown);
    }
    fs.writeFileSync(path.resolve(obj.folder.output.path,obj.fileData.name + extname), markdown);
  });
}//saveToFile
...
pmd.generateToc = function(config, objs){
  ...
  if (config.toc.template){
    ...
    objs.forEach(function(obj){
      ...
      file.fileName = obj.fileData.name + extname;
